...
SNP<-read.csv('TGCP_Ho_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$TGCP.Ho.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Observed Heterozygosity (Ho) Estimates', main='Timmons Grove County Park (TGCP)')
means <- tapply(SNP$TGCP.Ho.estimates,SNP$Subsampling.group,mean)
