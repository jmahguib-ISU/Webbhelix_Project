...
SNP<-read.csv('STT_Ho_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$STT.Ho.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Observed Heterozygosity (Ho) Estimates', main='Steamboat Trace Trail (STT)')
means <- tapply(SNP$STT.Ho.estimates,SNP$Subsampling.group,mean)
