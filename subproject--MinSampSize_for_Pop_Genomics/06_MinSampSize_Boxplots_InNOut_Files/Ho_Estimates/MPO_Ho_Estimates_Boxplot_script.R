...
SNP<-read.csv('MPO_Ho_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$MPO.Ho.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Observed Heterozygosity (Ho) Estimates', main='Mandan Park, Omaha (MPO)')
means <- tapply(SNP$MPO.Ho.estimates,SNP$Subsampling.group,mean)
