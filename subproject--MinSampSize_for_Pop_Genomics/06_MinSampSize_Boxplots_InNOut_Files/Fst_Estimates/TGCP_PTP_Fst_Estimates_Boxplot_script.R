...
SNP<-read.csv('TGCP_PTP_Fst_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$TGCP.PTP.Fst.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Fixation Index (Fst) Estimates', main='TGCP - PTP')
means <- tapply(SNP$TGCP.PTP.Fst.estimates,SNP$Subsampling.group,mean)
