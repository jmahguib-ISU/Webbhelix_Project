...
SNP<-read.csv('STT_MPO_Fst_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$STT.MPO.Fst.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Fixation Index (Fst) Estimates', main='STT - MPO')
means <- tapply(SNP$STT.MPO.Fst.estimates,SNP$Subsampling.group,mean)
