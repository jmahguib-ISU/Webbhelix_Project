...
SNP<-read.csv('TGCP_MPO_Fst_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$TGCP.MPO.Fst.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Fixation Index (Fst) Estimates', main='TGCP - MPO')
means <- tapply(SNP$TGCP.MPO.Fst.estimates,SNP$Subsampling.group,mean)
