...
SNP<-read.csv('STT_PTP_Fst_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$STT.PTP.Fst.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Fixation Index (Fst) Estimates', main='STT - PTP')
means <- tapply(SNP$STT.PTP.Fst.estimates,SNP$Subsampling.group,mean)
