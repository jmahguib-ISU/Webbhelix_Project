...
SNP<-read.csv('STT_Gis_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$STT.Gis.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Inbreeding Coefficient (Gis) Estimates', main='Steamboat Trace Trail (STT)')
means <- tapply(SNP$STT.Gis.estimates,SNP$Subsampling.group,mean)
