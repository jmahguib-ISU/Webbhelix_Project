...
SNP<-read.csv('PTP_Gis_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$PTP.Gis.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Inbreeding Coefficient (Gis) Estimates', main='Pikes Timber Park (PTP)')
means <- tapply(SNP$PTP.Gis.estimates,SNP$Subsampling.group,mean)
