...
SNP<-read.csv('MPO_Gis_SubSamp_Estimates_Boxplot_input.csv')
head(SNP)
boxplot(SNP$MPO.Gis.estimates~SNP$Subsampling.group, xlab='Subsampling Treatment (# individuals per population)', ylab='Inbreeding Coefficient (Gis) Estimates', main='Mandan Park, Omaha (MPO)')
means <- tapply(SNP$MPO.Gis.estimates,SNP$Subsampling.group,mean)
